...
import { audioUseCasePresets, audioBenchmarkExplanations } from '@/lib/audio-data'; // 오디오 데이터 임포트
import useSWR from 'swr';

const fetcher = (url: string) => fetch(url).then(res => res.json());

export default function AudioModelsPage() {
  const [selectedUseCase, setSelectedUseCase] = useState<string>('realistic-audio'); // 기본 사용 목적 (오디오)

  // 오디오 모델 API 라우트에서 데이터 가져오기
  const { data: evaluations, error, isLoading } = useSWR<ModelEvaluation[]>('/api/audio-models', fetcher);
  ...
  if (error) {
    return (
      <main className="p-6 max-w-6xl mx-auto flex justify-center items-center h-screen text-red-600">
        <p>오디오 모델 데이터를 가져오는데 실패했습니다: {error.message}</p>
      </main>
    );
  }
      ...
      <main className="max-w-7xl mx-auto py-6 sm:px-6 lg:px-8">
        ...
        <div className="px-4 py-6 sm:px-0 mb-6">
          <div className="bg-white rounded-lg shadow p-6 border border-blue-100">
            <h2 className="text-xl font-semibold text-blue-600 mb-2">오디오 생성 AI 모델 선택 가이드</h2>
            <p className="text-gray-600">
              이 페이지에서는 다양한 오디오 생성 AI 모델들의 성능을 비교하고, 사용 목적에 맞는 모델을 추천해드립니다.
            </p>
          </div>
        </div>

        {/* 사용 목적 선택 (오디오용 프리셋 사용) */}
        <div className="px-4 py-6 sm:px-0 mb-6">
          <div className="bg-white rounded-lg shadow p-6">
            <h2 className="text-xl font-semibold text-gray-900 mb-4">사용 목적 선택</h2>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
              {audioUseCasePresets.map((preset: UseCasePreset) => (
                <button
                  key={preset.id}
        ...
        {/* 모델 비교 테이블/차트 (공통 ModelTabs 컴포넌트 사용, 오디오 데이터 전달) */}
        <div className="px-4 py-6 sm:px-0 mb-6">
          <div className="bg-white rounded-lg shadow p-6">
            ...
            <p className="text-gray-600 mb-4 max-w-2xl">
              주요 오디오 생성 AI 모델들의 벤치마크 점수를 비교합니다. <b>FID-A, CLIP Score-A</b> 등 오디오 품질 및 프롬프트 일치도 지표를 포함합니다.
            </p>
            <ModelTabs
               evaluations={evaluations}
               useCasePresets={audioUseCasePresets}
               benchmarkExplanations={audioBenchmarkExplanations}
        ...
        <div className="px-4 py-6 sm:px-0">
          <div className="bg-white rounded-lg shadow p-6">
            ...
            <p className="text-gray-600 mb-6">
              각 평가 지표는 오디오 생성 모델의 다른 측면을 측정합니다.
            </p>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {audioBenchmarkExplanations.map((explanation: BenchmarkExplanation) => (
